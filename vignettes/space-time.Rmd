...
p_root <- "ecoscales"
p_dat <- "../external/data/result/"
p_fig <- paste0(p_root, "paper/figures")
p_til <- paste0(p_root, "external/data/misc")

datfile <- system.file("extdata/main-results.xlsx", package = "ecoscales")
calfile <- system.file("extdata/calibration-set.xlsx", package ="ecoscales")
allresfile <- system.file("extdata/all-results-notreuer.xlsx", 
                          package ="ecoscales")
dat <- data.table(read_excel(datfile))
cal <- data.table(read_excel(calfile))
full <- data.table(read_excel(allresfile))
...
# ilim <- range(resdat3$x)
# ilim <- round(rx / 0.5) * 0.5  # round to nearest 0.5
# ilim[1] <- -5  # set min interval to less than second
# ilim[2] <- 5.57  # set max interval to millenium
...
kernsl <- lapply(c(0.4, 0.7, 1), function(x) {
  rr <- kdensity(ilim[1], ilim[2], rlim[1], rlim[2], 0.1, resdat3, x)
  rr <- (rr / cellStats(rr, sum)) * 100
  er <- kdensity(dlim[1], dlim[2], elim[1], elim[2], 0.1, extdat3, x)
