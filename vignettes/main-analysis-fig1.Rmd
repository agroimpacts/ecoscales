...
```{r}
o <- sapply(1:length(vs), function(x) {  # x <- 1
  t(sapply(names(hdatstat_bt[-1]), function(y) {
    v <- hdatstat_bt[[y]][[vs[x]]]$mu
    round(cumsum(v) / sum(v) * 100, 1) 
  }))
})
```
...
Interval by observation type
```{r, echo = FALSE}
knitr::kable(o[[3]], col.names = tlab1[c(-1)])
```

Duration by observation type
```{r, echo = FALSE}
