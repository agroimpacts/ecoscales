...
vignette: >
  %\VignetteIndexEntry{Random Selection of Journal Titles}
...
library(lmisc)
library(data.table)
p_root <- fp(proj_root(proj.dir = "ecoscales"), "ecoscales")
# p_data <- fp(p_root, "external/data/draws")
...
# New calibration set randomly selected (note, run on earlier version of random draw taken before Elsen, 
# so to replicate use commented out drawsize and remove Elsen from reviewers before re-running)
...
fnames <- full_path(p_data, paste(reviewers, "2004_draw3.csv", sep = "_"))
...
for(i in 1:length(fn_old)) {
  ...
  neww <- read.csv(full_path(p_data, fn_new[i]))
...
for(i in 1:(length(fn_new) - 1)) {
  neww <- read.csv(full_path(p_data, fn_new[i]))
  newp1 <- read.csv(full_path(p_data, fn_new[i + 1]))
