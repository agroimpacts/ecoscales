...
\vspace{-0.5cm} 
  ...
  \Hitem \emph{\textbf{What is sampling duration, and how do we record it?}} How long an individual observation of an individual point in space took to make. Sampling duration multiplied by the number of repeats observations is used to calculate study duration. Often this value will not be reported, so you will have to use your best judgement, based on your knowledge of ecological methods, to approximate the duration. For example, for a field based method with intensive plot methods, if you can't estimate a plausible duration, assign a token 1 day. For remote sensing observations you can assume one second (the observations are effectively instantaneous).  
   \Hitem \emph{\textbf{What is the effective study duration, and how do estimate it?}} The effective study duration is the total period of time over which the phenomenon of interest was observed. More specifically, in the case of repeated observations, this is the total time that elapsed between when the first and last observations at a given point were made. For once-off (unreplicated) observations, this time is equivalent to the sample duration. However, there may be cases where once-off observations may have a longer effective duration then the sample duration.  For example, consider a study that counts occurrences of pollinators over three years, using transects that are located in different locations within the broader study area during each year \cite{rollin_differences_2013-1}.  The observations are therefore not strictly temporally replicated, but the authors control for year of collection in their subsequent analysis to avoid confounding effects.  In this case, we can consider the effective duration to be three years, as the temporal information is encoded in the analysis. 
  \Hitem \emph{\textbf{What is actual study duration, and how do we calculate it?}} The actual study duration is the integral of sampling duration, or the time spent making one observation of the phenomenon in question. To clarify, duration is the total time spent sampling/observing a single point in space--not the span of time between first and last sample (the effective duration), nor the integral of time spent in observing all spatial replicates. 
...
\hspace{-1cm}\underline{\textbf{Spatial scales:}}
  ...
  \Hitem \emph{\textbf{What is study extent, and how do I record it?}}  Broadly defined, actual study extent is the sampling resolution multiplied by the number of spatial replicates, divided by 10,000 to convert to hectares. For studies in which the spatial replicates are not spatially contiguous (as with most field-based studies), this means plot resolution (see Q16) multiplied by number of plots. For spatially contiguous studies (e.g., those based on remote sensing imagery), it should be the total area covered by the imagery, i.e., pixel resolution multiplied by the number of pixels. However, only record the area analyzed by the authors. If they used a sub-sample of pixels, the actual extent is the number of those pixels multiplied by pixel resolution. 
  
Effective survey extent is defined as the total extent bounding all spatial replicates, divided by 10,000 to convert to hectares. For studies in which spatial replicates are not spatially contiguous, this means the area of the minimum convex polygon bounding all spatial replicates. For example, if the study is conducted in a national park, the effective survey extent would be the area of the national park; if the study is conducted in three national parks, the effective survey extent would be the sum of the areas of all national parks. To calculate the effective survey extent, use the area of the study area/region given in the paper; when the area is not given, but when the survey region is given by name (e.g., Joshua Tree National Park or United States), look up the area through an online search and convert to hectares. When the area is not named, but a map is given, use an appropriate digitizing platform with a suitable map-providing backend (e.g. Google Earth Pro, QGIS with OpenLayers plugin) to navigate to the region and delineate a minimum convex polygon surrounding the plots/transects/sampling units to calculate the area in hectares. When studies focus on portions of survey regions (e.g., mangroves in a coastal National Park), attempt to delineate the focal portions (the mangroves) and not the larger survey region (entire National Park) using Google Earth Pro.

For spatially contiguous studies (e.g. those based on satellite imagery), the effective survey extent is the total area covered by the imagery (in such cases, effective survey extent equals actual survey extent). However, if spatially contiguous studies only use a sub-sample of pixels, the effective survey extent is the area of the minimum convex polygon bounding all pixels (calculated following the methods above).
