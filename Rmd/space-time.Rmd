...
p <- c(0.58, 0.75, 0.50, 1.01, 1.23, 0.59)  # from calibration.Rmd
...
pdf("paper/figures/hists.pdf", width = 5, height = 5)
...
plot(h, freq = FALSE, ylab = "", las = 2,  xaxt = "n", mgp = c(3.25, 0.25, mga),
     ...
     ylim = c(0, yl[1]), main = "", col = reds[4], cex.lab = cxl)
...
# extent
h <- hist(hdat$ext, breaks = aaxis2$logarea, plot = FALSE)
# h <- hist(hdat$aext, breaks = c(-6, aaxis2$logarea), plot = FALSE)
h$density <- h$counts / sum(h$counts) * 100
plot(h, freq = FALSE, ylab = "", las = 2,  xaxt = "n", mgp = c(3.25, 0.25, mga),
     tcl = -0.2, cex.axis = cxa, xlab = "Spatial extent", ylim = c(0, yl[1]), 
     main = "", col = reds[6], cex.lab = cxl)
axis(1, at = aaxis2$logarea,
     labels = alab2,
     # labels = c(expression(paste(NULL<="0.01m"^2)), expression(paste(0.1, m^2)),                 alab2[-1]), 
     las = 2, cex.axis = cxa,
     tcl = -0.2, mgp = c(2, 0.25, mga))
...
h$density <- h$counts / sum(h$counts) * 100
plot(h, freq = FALSE, ylab = "", las = 2,  xaxt = "n", mgp = c(3.25, 0.25, mga),
     tcl = -0.2, cex.axis = cxa, xlab = "Sampling interval", ylim = c(0, yl[2]), 
...
h$density <- h$counts / sum(h$counts) * 100
plot(h, freq = FALSE, ylab = "", las = 2,  xaxt = "n", mgp = c(3.25, 0.25, mga),
     tcl = -0.2, cex.axis = cxa, xlab = "Temporal duration", ylim = c(0, yl[2]),      main = "", col = blues[6], cex.lab = cxl)
...
#                         expression(paste(0.1, m^2)), alab2[-c(1:2)]),
...
datf[like(study_type, "auto") & log10(t_btwn_samp) < -1.380211, .N] / 
  length(which(hdat[["int"]] < -1.380211))  # 78 % high frequency obs automated 
datf[like(study_type, "remote|other"), .N] / nrow(datf) # 7 % remote
...
### Figure 3
```{r}
datf[, plot(log10(eff_ext), log10(act_ext))]
datf[, plot(log10(act_dur), log10(eff_dur))]
datf[log10(eff_dur) < log10(act_dur)]
calr[log10(eff_dur) < log10(act_dur)]
dat[DOI == "10.1007/s10021-013-9684-x"]
cal[DOI == "10.1007/s00442-010-1713-2"]
dat[DOI %in% datf[log10(eff_ext) < log10(act_ext), DOI]]
dat[DOI %in% datf[log10(eff_ext) < log10(act_ext), DOI]]
datf[log10(eff_ext) < log10(act_ext), ]
datf[DOI == "10.1111/j.1466-8238.2011.00658.x", 
     .(log10(round(eff_ext)), log10(round(act_ext)))]
datf[DOI == "10.1111/j.1466-8238.2011.00658.x", 
     .(round(eff_ext), round(act_ext))]
...
# Bootstrap version
vs1 <- c("eff_ext", "act_ext", "eff_dur", "act_dur")
vs2 <- c("eext", "aext", "edur", "adur")
hdat_bs2 <- lapply(1:length(bootperturb), function(x) {  # x <- 1
  ...
  edb[, dif := eext - aext]
  # elm <- edb[, lm(dif ~ aext)]
  # preds <- predict(elm, newdata = data.frame(aext = extdum))
  # plot(extdum, preds)
...
edif_int <- rbindlist(lapply(hdat_bs2, function(y) {  # y <- hdat_bs2[[2]]
  ...
  # dd[, list("int" = findInterval(aext, eint), "dif" = dif)]
  dd[, int := findInterval(aext, eint)]
...
ewgt * 100
ealphs <- (ewgt - min(ewgt)) / diff(range(ewgt))
...
dalphs <- (dwgt - min(dwgt)) / diff(range(dwgt))
...
# plot(1:length(cols), col = lcols, pch = 20)
...
for(i in 1:length(ebox)) {
  boxplot_v(bx[i], y = ebox[[i]], n = 1.75, whiskhgt = 1.5, bfill = cols[i])
} 
axis(1, at = eint, labels = alab2, las = 2, tcl = -0.2, cex.axis = cxa, 
     mgp = c(3, 0.5, 0))
axis(2, at = seq(0, 14, 2), las = 2, tcl = -0.2, cex.axis = cxa, 
     mgp = c(3, 0.5, 0))
mtext("Actual Extent", side = 1, line = 3)
mtext("Difference Magnitude:", side = 2, line = 2)
mtext("Effective minus Actual", side = 2, line = 1)
# xs <- seq(2, 10, by = (10 - 2) / (length(ealphs)))
# ealphss <- sort(ealphs, decreasing = TRUE)
# lcols <- sapply(ealphss, function(x) rgb(0, 0.1, 1, alpha = x))
# rect_shade(xs, y = c(13, 14), fillcol = lcols, linecol = "grey")
# text(xs, rep(13, length(xs)), sort(round(ewgt * 100), decreasing = TRUE),
#      srt = 90)

bx <- rowMeans(cbind(dint[-length(dint)], dint[-1]))
bd <- abs(dint[-length(dint)] - dint[-1])
cols <- sapply(dalphs, function(x) rgb(1, 0, 0.1, alpha = x))
# plot(1:length(cols), col = cols, pch = 20)
plot(tx, c(0, 14), pch = "", xlab = "", ylab = "", xaxt = "n", 
     mgp = c(3, 0.5, 0), tcl = -0.2, yaxt = "n")
for(i in 1:length(tbox)) {
  boxplot_v(bx[i], y = tbox[[i]], n = 0.9, inhgt = bd[i] * 10, whiskhgt = 0.5, 
            bfill = cols[i])
} 
...
mtext("Actual Duration", side = 1, line = 3)
dev.off()

# # Calculate predictions from ensemble of regression coefficients
# extdum <- seq(-10, 10, 1)
# extpred <- do.call(rbind, lapply(1:nrow(hdat_bs2_elm), function(x) {
#   hdat_bs2_elm[x, 1] + hdat_bs2_elm[x, 2] * extdum
# }))
# extconf <- apply(extpred, 2, function(x) c(mean(x), quantile(x, c(0.025, 0.975))))
# durdum <- seq(-5, 5, 1)
# durpred <- lapply(1:nrow(hdat_bs2_tlm), function(x) {
#   hdat_bs2_tlm[x, 1] + hdat_bs2_tlm[x, 2] * durdum
# })
# 
# plot(extmu[, 1], extmu[, 2], col = "grey", pch = 20)
# lines(extdum, extconf[1, ])
# lines(extdum, extconf[2, ])
# 
# 
# datf[, plot(log10(act_ext), log10(eff_ext - act_ext))]
# 
# log10(0.1 - 0.05)
# log10(0.1) - log10(0.05)
# a <- seq(0.1, 10, 0.1)^10
# mv <- runif(length(a), min = 1 - 0.4, max = 1 + 0.4)
# b <- sapply(1:length(a), function(x) a[x] * mv[x])
# plot(a, b)
# cbind(log10(a), log10(b), log10(a) - log10(b), a, b, log10(a - b))
# plot(log10(a), log10(b))
# 
# datf2 <- copy(datf[, .(act_ext, eff_ext)])
# chkd <- datf2[order(act_ext), {
#    ae = round(act_ext, 6)
#    ee = round(eff_ext, 2) 
#    aext = round(log10(act_ext), 2)
#    eext = round(log10(eff_ext), 2)
#    dif = round(ee - ae, 6)
#    dlog = round(log10(eff_ext) - log10(act_ext), 2)
#    logd = round(log10(eff_ext - act_ext), 2)
#    list("ae" = ae, "ee" = ee, "aext" = aext, "eext" = eext, 
#         "dif" = dif, "dlog" = dlog, "logd" = logd)
# }]
# chkd[1:100, ]
# chkd[, plot(aext, logd)]
# 
# plot(durmu[, 1], durmu[, 2])
# plot(extmu[, 1], extmu[, 2])
