...
# old function, retained here for easy recollection of first approach
...
colSums(senst) / nrow(senst) * 100

```

### Varying kernel sizes on resolution versus extent
```{r,eval = FALSE}
# resolution data
...
lapply(kernsl, function(x) {
  ...
  axis(2, at = aaxis2$logarea, labels = alab2, las = 2)
  aargs <- list(mgp = c(3, 0.25, 0), at = brklabs2, labels = brklabs2, 
                cex.axis = cxa, tcl = -0.1)
