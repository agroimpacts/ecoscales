...
p <- c(0.58, 0.72, 0.41, 1.01, 1.25, 0.53)  # from calibration.Rmd
...
# effective extent
### fix this here to have label for 10^-6 (so that limit isn't hard-coded below)
...
### fix this here to have value for 10^-6 (so that limit isn't hard-coded below)
...
datf[t_btwn_samp == 0, .N] / datf[, .N]  # 38% of samples are once-offs
...
### fix in scaling section (so that min extent limit isn't hard-coded)
vle <- c(-6, range(aaxis2$logarea)[2])  ### fix
...
hdat_bs <- lapply(1:length(bootperturb), function(x) {  # x <- 1
  ...
  ### fix in scaling section (so that this limit isn't hard-coded)
...
h$density <- h$counts / sum(h$counts) * 100
plot(h, freq = FALSE, ylab = "", las = 2,  xaxt = "n", mgp = c(3.25, 0.25, mga),
     tcl = -0.2, cex.axis = cxa, xlab = "Temporal duration", ylim = c(0, yl[2]), 
