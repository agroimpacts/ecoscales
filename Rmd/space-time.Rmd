...
dat2 <- copy(dat[, .(st, plot_res, n_sites, sampled_area, samp_duration,
                     t_btwn_samp, study_duration)])
rnms <- c("st", "DOI", "feature", "observer", "plot_res", "n_sites", 
          "sampled_area", "t_btwn_samp", "study_duration")
calr <- cal[, rnms, with = FALSE]
for(j in names(calr)[c(3, 5:9)]) set(calr, j=j, value = as.numeric(calr[[j]]))
...
setcolorder(calr, names(calr)[c(1:4, 7, 5:6)])
...
pdf("paper/figures/kde4.pdf", width = 7, height = 5.5)
par(mfrow = c(2, 2), mar = c(6, 6, 0.3, 3), oma = c(0, 0, 0.5, 1), 
    mgp = c(2, 0.25, 0))
# resolutions
# image(rres3, col = rev(terrain.colors(30)), breaks = brks,
image(rres3, col = rev(terrain.colors(50)), axes = FALSE, xlab = "", ylab = "")
...
plot(rres3, legend.only = TRUE, axis.args = aargs, legend.width = 1.25,
     #legend.shrink = 0.9, breaks = brks, col = rev(terrain.colors(30)))
     legend.shrink = 0.9, col = rev(terrain.colors(50)))
mtext("A", side = 3, line = -1, cex = 0.8, adj = 0.05)
# points(resdat3$x[which(datf$st == 4)], resdat3$y[which(datf$st == 4)], 
#        pch = 20, col = "red", cex = 0.5)
# points(resdat3$x[which(datf$st == 3)], resdat3$y[which(datf$st == 3)], 
#        pch = 20, col = "blue", cex = 0.5)
# points(resdat3$x[which(datf$st == 5)], resdat3$y[which(datf$st == 5)], 
#        pch = 20, col = "blue", cex = 0.5)

# extents
# image(rext3, col = rev(terrain.colors(30)), axes = FALSE, xlab = "", 
image(rext3, col = rev(terrain.colors(50)), axes = FALSE, xlab = "", 
      ylab = "")#, breaks = brks2)
axis(1, at = taxis2$logdays[-5], labels = tlab2[-5], las = 2,
     cex.axis = cxa, tcl = -0.2)
axis(2, at = aaxis2$logarea, labels = alab2, las = 2, cex.axis = cxa,
...
# spatial
image(sres, col = rev(terrain.colors(100)), axes = FALSE, xlab = "", ylab = "")
...
mtext("Spatial extent", 2, cex = cxa, line = ll)
aargs <- list(mgp = c(3, 0.25, 0), at = seq(0, 0.12, 0.02), 
              labels = seq(0, 0.12, 0.02), cex.axis = cxa, tcl = -0.1)
plot(sres, legend.only = TRUE, axis.args = aargs, legend.width = 1.25,
     legend.shrink = 0.9, col = rev(terrain.colors(100)))
mtext("C", side = 3, line = -1, cex = 0.8, adj = 0.05)

# temporal
image(tres, col = rev(terrain.colors(50)), axes = FALSE, xlab = "", ylab = "")
# points(spatdat$x, spatdat$y, cex = 0.1)
axis(1, at = taxis1$logdays[-c(5, 10)], labels = tlab1[-c(5, 10)], 
     las = 2, tcl = -0.2, cex.axis = cxa)
...
par(mfrow = c(1, 4), mar = c(4, 1, 1, 1), oma = c(2, 3, 0, 0))
# resolution
h <- hist(hdat$res, breaks = aaxis1$logres, plot = FALSE)
h$density <- h$counts / sum(h$counts) * 100
plot(h, freq = FALSE, ylab = "", las = 2,  xaxt = "n", 
     xlab = "Spatial resolution", ylim = c(0, 25), main = "", col = "red1")
axis(1, at = aaxis1$logres, labels = alab1, las = 2, cex.axis = cxa, 
     tcl = -0.2, mgp = c(2, 0.25, -0.45))
mtext("Percent of observations", side = 2, line = 3, cex = 0.8)
mtext("A", side = 3, line = -1.5, cex = 0.8, adj = 0.05)

# extent
h <- hist(hdat$ext, breaks = c(-6, aaxis2$logarea), plot = FALSE)
h$density <- h$counts / sum(h$counts) * 100
plot(h, freq = FALSE, ylab = "", las = 2,  xaxt = "n", 
     xlab = "Spatial extent", ylim = c(0, 20), main = "", col = "red3")
axis(1, at = c(-6, aaxis2$logarea), 
     labels = c(expression(paste(NULL<="0.01m"^2)), expression(paste(0.1, m^2)), 
                alab2[-1]), las = 2, cex.axis = cxa, 
     tcl = -0.2, mgp = c(2, 0.25, -0.45))
mtext("B", side = 3, line = -1.5, cex = 0.8, adj = 0.05)
...
h$density <- h$counts / sum(h$counts) * 100
plot(h, freq = FALSE, ylab = "", las = 2,  xaxt = "n", 
     xlab = "Sampling interval", ylim = c(0, 30), main = "", col = "blue1")
axis(1, at = tax, labels = tlab1[-10], las = 2, cex.axis = cxa, 
     tcl = -0.2, mgp = c(2, 0.25, -0.45))
mtext("C", side = 3, line = -1.5, cex = 0.8, adj = 0.05)
...
h$density <- h$counts / sum(h$counts) * 100
plot(h, freq = FALSE, ylab = "", las = 2,  xaxt = "n", 
     xlab = "Temporal duration", ylim = c(0, 30), main = "", col = "blue4")
axis(1, at = taxis2$logdays, labels = tlab2, las = 2, cex.axis = cxa, 
     tcl = -0.2, mgp = c(2, 0.25, -0.45))
mtext("D", side = 3, line = -1.5, cex = 0.8, adj = 0.05)
...
sum(hdatstat[[1]]$density[1:6])  # 52 % between 10cm^2 and 1 m^2; 63% < 1m^2
sum(hdatstat[[1]]$density[7:10]) # 25 % between 1 m^2 and 10000 m^2
sum(hdatstat[[1]]$density[11:(l)]) # 12 % 1-100 ha
# dat[plot_res >= 10000^2]
aaxis1

# extent
l <- length(hdatstat[[2]]$density)
sum(hdatstat[[2]]$density[1:7])  # 85 % < 10 ha
sum(hdatstat[[2]]$density[1:2])  # 30 % < 1 m^2
sum(hdatstat[[2]]$density[10:l]) # 7 % covered an area > 10 ha
sum(hdatstat[[2]]$density[14:l]) # 7 % covered an area > 10 ha

# interval
l <- length(hdatstat[[3]]$density)
sum(hdatstat[[3]]$density[l])  # 30% are once-offs
sum(hdatstat[[3]]$density[1:3])  # 24 % < daily
sum(hdatstat[[3]]$density[4:5])  # 20% daily up to monthly
sum(hdatstat[[3]]$density[5:6])  # 35% monthly up to yearly
sum(hdatstat[[3]]$density[7:9]) # 4 % yearly to decadal

# duration
l <- length(hdatstat[[4]]$density)
sum(hdatstat[[4]]$density[1:3])  # 59 % up to 1 day
sum(hdatstat[[4]]$density[4:6])  # 35% covered between 1 day and 1 year
sum(hdatstat[[4]]$density[7:10]) # 6% year to century

# datf[, plot(log10(n_sites), log10(plot_res))]
dat[like(study_type, "field"), plot(log10(n_sites), log10(plot_res))]
dat[like(study_type, "field"), plot(log10(plot_res), log10(sampled_area), 
                                    ylim = c(-10, 10))]

dat[like(study_type, "remote"), 
    points(log10(plot_res), log10(sampled_area), pch = 20, col = "red")]
dat[like(study_type, "automated"), 
