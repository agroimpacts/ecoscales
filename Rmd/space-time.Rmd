...
datf[study_duration == samp_duration, .N] / nrow(datf)
datf[(study_duration == samp_duration) & t_btwn_samp != 0]
datf[(study_duration != samp_duration) & t_btwn_samp == 0]
...
hdat_bs <- lapply(1:length(bootperturb), function(x) {  # x <- 1
  print(x)
  DT <- bootperturb[[x]]
  hdb <- cbind("res" = log10(DT$plot_res), "ext" = log10(DT$sampled_area), 
               "int" = log10(to_infin_byond(DT$t_btwn_samp)), 
               "dur" = log10(DT$study_duration))
  hdb <- data.table(hdb)
...
# extent
extdat3 <- data.table(x = log10(kdat$study_duration), 
                      y = log10(kdat$sampled_area))
...
# spatial framing
spatdat <- data.table(x = log10(kdat$plot_res), y = log10(kdat$sampled_area))
...
tempdat2 <- data.table(x = log10(tbtwn), y = log10(datf$study_duration))
